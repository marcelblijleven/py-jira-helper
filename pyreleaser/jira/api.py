...
class Api:
    ...
    def create_fix_version(self, version: str, released: bool = False) -> None:
    ...
    def get_ticket_versions(self, issue: str) -> list[FixVersion]:
        endpoint = self.host + self.api_endpoint + '/issue/' + issue + '?fields=fixVersions'
        ...
        if not (fields := data.get('fields')) or not (versions := fields.get('fixVersions')):
            return []
